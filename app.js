...
function render() {
  ...
  app.innerHTML = `<p class="course-meta">TEXTBOOK · ${esc(prompts[0]?.textbook || 'Not specified')}</p><div class="prompt-grid">${prompts.map(item => `<article class="prompt"><div class="preview">${item.status === 'ready' && item.video ? `<video controls preload="metadata" src="${esc(item.video)}"></video>` : '<span class="generating">VIDEO COMING SOON</span>'}</div><div class="prompt-body"><span class="status">${item.status === 'ready' ? 'VIDEO READY' : 'VIDEO COMING SOON'}</span><h2>${esc(item.title)}</h2><div class="tags">${(item.tags || []).filter(Boolean).map(tag => `<span class="tag">${esc(tag)}</span>`).join('')}</div><div class="prompt-text">${esc(item.prompt)}</div><a class="cta" href="https://leadde.ai/animation" target="_blank" rel="noreferrer">Create Your Own Animation with Leadde →</a></div></article>`).join('')}</div>`;
}
